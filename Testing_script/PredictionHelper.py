...
class PredictionHelper:
    ...
    def __init__(self, cnn_model):
        ...
        self.species_folder = your_folder_directory+'/generative_data_augmentation_gibbons_copy/Testing_script/'
        self.lowpass_cutoff = 2000  # Cutt off for low pass filter
        self.downsample_rate = 4800  # Frequency to downsample to
        self.nyquist_rate = 2400  # Nyquist rate (half of sampling rate)
        ...
        self.n_ftt = 2024  # Hann window length
        self.hop_length = 151  # Sepctrogram hop size
        self.n_mels = 128  # Spectrogram number of mells
        self.f_min = 4000  # Spectrogram, minimum frequency for call
        self.f_max = 9000  # Spectrogram, maximum frequency for call
        self.audio_path = your_folder_directory+'/generative_data_augmentation_gibbons_copy/Testing_script/Audio/'
        self.annotations_path = your_folder_directory+'/generative_data_augmentation_gibbons_copy/Testing_script/Annotations/'
        self.testing_files = your_folder_directory+'/generative_data_augmentation_gibbons_copy/Testing_script/DataFiles/TestingFiles.txt'

        print("printing the testing files", self.testing_files)
    ...
    def load_model(self):
        ...
        if self.cnn_model == 'custom_cnn':
            ...
            model = networks.mobile_net()
        print('Loading weights: ')
        checkpoint = your_folder_directory+f'/generative_data_augmentation_gibbons_copy/cnn_checkpoint/checkpoint/{base_name}/cp.ckpt'
    ...
    def predict_all_test_files(self, verbose):
        ...
        for testing_file in testing_files.values:
            ...
            if self.audio_path+file_name_no_extension+'.wav' in glob.glob(self.audio_path+"*.wav"):
                ...
                #cumulative_probs_absence.extend(values[:,0])
                #cumulative_probs_presence.extend(values[:,1])
                ...
                if len(predictions) > 0:
                    ...
                    # Write the .svl file
                    #text_file = open(self.species_folder+'ModelPredictions/'+f'{base_name}/'+file_name_no_extension+".svl", "w")
