...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self , dim_z , name="encoder" , **kwargs):
        ...
        self.dim_z = dim_z
        self.conv_layer1 = tf.keras.layers.Conv2D(filters=32 , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
        self.conv_layer2 = tf.keras.layers.Conv2D(filters=64 , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
...
class Decoder(tf.keras.layers.Layer):
    ...
    def __init__(self , dim_z , name="encoder" , **kwargs):
        ...
        self.dim_z = dim_z
        self.dense1 = tf.keras.layers.Dense(units=32 * 32 * 64 , activation='relu')
        self.reshape_layer = tf.keras.layers.Reshape((32 , 32 , 64))
        self.conv_transpose_layer1 = tf.keras.layers.Conv2DTranspose(filters=64 , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
        self.conv_transpose_layer2 = tf.keras.layers.Conv2DTranspose(filters=32 , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
        self.conv_transpose_layer3 = tf.keras.layers.Conv2DTranspose(filters=1 , kernel_size=3 , strides=1 ,
                                                                     padding='same')
...
class shallow_vae(tf.keras.Model):

    def __init__(self , dim_z , name="autoencoder" , **kwargs):
        super(shallow_vae , self).__init__(name=name , **kwargs)
        self.dim_x = (128 , 128 , 1)
        self.dim_z = dim_z
    ...
    def call(self , x_input):
        ...
        kl_divergence = 0.01*tf.math.reduce_mean(kl_divergence)
